...
startStopPub = rospy.Publisher('/license_plate', String, queue_size = 1)
#image_pub = rospy.Publisher('DebugScreen', Image, queue_size=1)

rospy.sleep(1)
while(rospy.get_time() < 10):
	print("Getting time sync")

ENDTIMEMIN = 4.0

print("starting")
startString = str('TeamRed,multi21,0,XXXX')
endString = str('TeamRed,multi21,-1,XXXX')
startStopPub.publish(startString)
timeLeft = True
...
while not rospy.is_shutdown():
	...
	while data is None:
		try:
			data = rospy.wait_for_message('/R1/pi_camera/image_raw', Image, timeout=10)
		except:
			pass
	...
	cv2.putText(cv_image, 'R:'+str(rows)+'  C:'+str(cols) +'  T(s):' + str(curTime), org, font, fontScale, color, thickness, cv2.LINE_AA)
	...
	if((curTime-startTime) > ENDTIMEMIN*60.0):

		velocity = 0.0
		omega = 0.0
		
		if(timeLeft):
			startStopPub.publish(endString)
			timeLeft = False
